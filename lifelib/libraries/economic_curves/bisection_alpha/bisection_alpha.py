...
def Galfa(M_Obs: np.ndarray, r_Obs: np.ndarray, ufr, alpha, Tau):
    ...
    """
    from smith_wilson_funcs import SWCalibrate

    U = max(M_Obs)  # Find maximum liquid maturity from input
    T = max(
        U + 40, 60
    )  # Define the convergence point as defined in paragraph 120 and again in 157
    C = np.identity(
        M_Obs.size
    )  # Construct cash flow matrix described in paragraph 137 assuming the input is ZCB bonds with notional value of 1
    d = np.exp(
        -np.log(1 + ufr) * M_Obs
    )  # Calculate vector d described in paragraph 138
    Q = np.diag(d) @ C  # Matrix Q described in paragraph 139
    b = SWCalibrate(
        r_Obs, M_Obs, ufr, alpha
    )  # Calculate the calibration vector b using the equation from paragraph 149
    K = (1 + alpha * M_Obs @ Q @ b) / (
        np.sinh(alpha * M_Obs.transpose()) @ Q @ b
    )  # Calculate kappa as defined in the paragraph 155
    return (
        alpha / np.abs(1 - K * np.exp(alpha * T)) - Tau
    )  # Size of the gap at the convergence point between the alowable tolerance Tau and the actual curve. Defined in paragraph 158
...
def BisectionAlpha(xStart, xEnd, M_Obs, r_Obs, ufr, Tau, Precision, maxIter):
    ...
    """
    yStart = Galfa(
        M_Obs, r_Obs, ufr, xStart, Tau
    )  # Check if the initial point is a solution
    ...
    while iIter <= maxIter:
        ...
        if yMid == 0 or (xEnd - xStart) / 2 < Precision:  # Solution found
            ...
            if np.sign(yMid) == np.sign(
                yStart
            ):  # If the start point and the middle point have the same sign, then the root must be in the second half of the interval
                xStart = xMid
