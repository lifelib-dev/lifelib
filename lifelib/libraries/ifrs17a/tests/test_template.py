...
def test_best_estimate(ifrsvars):
    ...
    vars = ifrsvars
    filter = (
        (vars["Year"] == 2021) & (vars["Month"] == 3) & (vars["EstimateType"] == "BE")
    )
    df = (
        vars.loc[filter]
        .set_index(["Novelty", "AocType", "EconomicBasis", "LiabilityType"])["Value"]
    ...
    expected = pd.read_excel(
        os.path.join(resultdir, "template.xlsx"),
        sheet_name="BE",
        index_col=[0, 1],
        header=[0, 1],
    )
...
def test_risk_adjustment(ifrsvars):
    vars = ifrsvars
    filter = (
        (vars["Year"] == 2021) & (vars["Month"] == 3) & (vars["EstimateType"] == "RA")
    )
    df = (
        vars.loc[filter]
        .set_index(["Novelty", "AocType", "LiabilityType", "EconomicBasis"])["Value"]
        .groupby(level=list(range(4)))
        .sum()
        .unstack(level=[-1, -2])
        .fillna(0)
...
def test_written_actuals(ifrsvars):
    ...
    vars = ifrsvars
    filter = (
        (vars["Year"] == 2021) & (vars["Month"] == 3) & (vars["EstimateType"] == "A")
    )
    df = (
        vars.loc[filter]
        .set_index(["AocType", "AmountType", "LiabilityType"])["Value"]
        .groupby(level=list(range(3)))
        .sum()
        .unstack(level=-1)
        .fillna(0)
    )
...
def test_overdue_advanced_actuals(ifrsvars):
    ...
    filter = (
        (vars["Year"] == 2021)
        & (vars["Month"] == 3)
        & ((vars["EstimateType"] == "OA") | (vars["EstimateType"] == "AA"))
    )
    df = (
        vars.loc[filter]
        .set_index(["AocType", "EstimateType"])["Value"]
        .groupby(level=list(range(2)))
        .sum()
        .unstack(level=-1)
        .fillna(0)
    )
...
def test_deferred_actuals(ifrsvars):
    ...
    vars = ifrsvars
    filter = (
        (vars["Year"] == 2021) & (vars["Month"] == 3) & (vars["EstimateType"] == "DA")
    )
    df = (
        vars.loc[filter]
        .set_index(["AocType", "EstimateType"])["Value"]
    ...
    expected = pd.read_excel(
        os.path.join(resultdir, "template.xlsx"), sheet_name="DA", index_col=0, header=0
    )
    pd.testing.assert_frame_equal(df, expected, check_names=False, check_dtype=False)
...
def test_technical_margin(ifrsvars):
    ...
    filter = (
        ...
        & (
            (vars["EstimateType"] == "C")
            | (vars["EstimateType"] == "L")
            | (vars["EstimateType"] == "LR")
        )
    )
    df = (
        vars.loc[filter]
        .set_index(["Novelty", "AocType", "EstimateType"])["Value"]
        .groupby(level=list(range(3)))
        .sum()
        .unstack(level=-1)
        .fillna(0)
