...
def test_present_values_ep2():
    ...
    df = (
        vars[(vars["EconomicBasis"] == "L") & (vars["EstimateType"] == "BE")]
        .set_index(["Novelty", "AocType", "AmountType"])["Value"]
        .groupby(level=[0, 1, 2])
        .sum()
        .unstack(level=2)
    )

    expected = pd.read_excel(os.path.join(resultdir, "pv_ep2.xlsx"), index_col=[0, 1])
...
def test_present_values(reporting_node, month):
    ...
    filter = (
        (vars["EconomicBasis"] == "L")
        & (vars["EstimateType"] == "BE")
        & (vars["ReportingNode"] == reporting_node)
        & (vars["Year"] == 2021)
        & (vars["Month"] == month)
    )
    ...
    expected = pd.read_excel(
        os.path.join(resultdir, "pv_ep3.xlsx"),
        sheet_name=reporting_node,
        index_col=[0, 1],
    )
