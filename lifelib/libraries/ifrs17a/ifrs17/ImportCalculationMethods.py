...
def GetReferenceAocStepForCalculated(
    identities: list[AocStep],
    aocConfigurationByAocStep: dict[AocStep, AocConfiguration],
    identityAocStep: AocStep,
) -> AocStep:

    for aocStep in reversed(identities):
        if (
            aocConfigurationByAocStep[aocStep].DataType != DataType.Calculated
            and aocConfigurationByAocStep[aocStep].DataType
            != DataType.CalculatedTelescopic
            and aocConfigurationByAocStep[aocStep].Order
            < aocConfigurationByAocStep[identityAocStep].Order
...
def GetPreviousIdentities(identities: list[AocStep]) -> dict[AocStep, list[AocStep]]:
    ...
    previousStep = {
        n: AocStep(AocTypes.BOP, n) if n in bopNovelties else None
        for n in [Novelties.N, Novelties.I, Novelties.C]
    }
    ...
    for id_ in identities:
        if id_.AocType != AocTypes.BOP:
            ret = (
                [v for v in previousStep.values() if v]
                if id_.AocType == AocTypes.CL
                else [previousStep[id_.Novelty]] if previousStep[id_.Novelty] else []
            )
