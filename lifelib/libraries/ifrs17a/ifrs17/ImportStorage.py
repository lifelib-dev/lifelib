...
class ImportStorage:
    ...
    def __init__(self, args: ImportArgs, database: IfrsDatabase):
        ...
        self.AmountTypeDimension = {
            x.SystemName: x for x in self.database.Query(AmountType)
        }
        self.NoveltyDimension = {x.SystemName: x for x in self.database.Query(Novelty)}

        self.ExperienceAdjustEstimateTypeMapping = {
            EstimateTypes.A: EstimateTypes.APA
        ...
        for x in temp:
            temp2[str(x)] = {
                et.SystemName for et in estimateTypes if x in et.InputSource
            }
        ...
        self.ProjectionConfiguration = sorted(
            [
                x
                for x in self.database.Query(ProjectionConfiguration)
                if x.Shift > 0
                or x.TimeStep == self.args.Month
                or (
                    x.TimeStep > self.args.Month
                    and x.TimeStep % self.periodicityInMonths == 0
                )
            ],
        ...
        if (
            len(
                temp := [
                    p
                    for p in self.database.Query(PartitionByReportingNodeAndPeriod)
                    if p.ReportingNode == self.args.ReportingNode
                    and p.Year == self.CurrentReportingPeriod.Year
                    and p.Month == self.CurrentReportingPeriod.Month
        ...
        self.database.Partition.Set(
            PartitionByReportingNodeAndPeriod, self.TargetPartition
        )

        # Get data from Workspace (result of parsing)

        parsedRawVariables = self.database.Query(
            RawVariable, partition=self.TargetPartition
        )
        parsedIfrsVariables = self.database.Query(
            IfrsVariable, partition=self.TargetPartition
        )
        ...
        self.InterDataNodeParametersByGoc = self.database.LoadInterDataNodeParameters(
            self.args
        )

        primaryScopeFromParsedVariables = (
            set(x.DataNode for x in parsedRawVariables)
            if self.ImportFormat == ImportFormats.Cashflow
            else set(x.DataNode for x in parsedIfrsVariables)
        )

        temp = set(
            goc
            for goc in primaryScopeFromParsedVariables
            if not self.DataNodeDataBySystemName[goc].IsReinsurance
            and self.DataNodeDataBySystemName[goc].LiabilityType == LiabilityTypes.LRC
        )

        primaryScopeFromLinkedReinsurance = set()
        for goc in temp:
            if interDataNodeParamByPeriod := self.InterDataNodeParametersByGoc.get(
                goc, None
            ):
                temp2 = set(
                    param.LinkedDataNode if param.DataNode == goc else param.DataNode
                    for param in interDataNodeParamByPeriod[CurrentPeriod]
                    if goc not in primaryScopeFromParsedVariables
                )
            else:
                temp2 = set()
        ...
        primaryScope = (
            primaryScopeFromParsedVariables | primaryScopeFromLinkedReinsurance
        )

        temp = {
            k: v
            for k, v in self.InterDataNodeParametersByGoc.items()
            if k in primaryScope
        }
        secondaryScope = set()
        for key, val in temp.items():
            temp = set(
                param.LinkedDataNode if param.DataNode == key else param.DataNode
                for param in val[CurrentPeriod]
            )
            secondaryScope |= set(goc for goc in temp if goc not in primaryScope)

        allImportScopes = primaryScope | secondaryScope
        self.DataNodesByImportScope = {
            ImportScope.Primary: primaryScope,
            ImportScope.Secondary: secondaryScope,
        }

        # Parameters

        self.PartnerRating = self.database.LoadCurrentAndPreviousParameter(
            PartnerRating, self.args, lambda x: x.Partner
        )
        self.CreditDefaultRates = self.database.LoadCurrentAndPreviousParameter(
            CreditDefaultRate, self.args, lambda x: x.CreditRiskRating
        )
        self.SingleDataNodeParametersByGoc = self.database.LoadSingleDataNodeParameters(
            self.args
        )
        self.LockedInYieldCurve = self.database.LoadLockedInYieldCurve(
            self.args, [self.DataNodeDataBySystemName[dn] for dn in allImportScopes]
        )
        self.CurrentYieldCurve = self.database.LoadCurrentYieldCurve(
            self.args, [self.DataNodeDataBySystemName[dn] for dn in allImportScopes]
        )  # TODO Rename this variable
        self.AocConfigurationByAocStep = (
            self.database.LoadAocStepConfigurationAsDictionary(
        ...
        for x in temp:
            self.aocStepByInputSource[x] = {
                k
                for k, v in self.AocConfigurationByAocStep.items()
                if x in v.InputSource
            }
        ...
        allImportScopesAtInceptionYear = {
            dnd.DataNode for dnd in temp if dnd.Year == self.args.Year
        }
        allImportScopesNotAtInceptionYear = (
            allImportScopes - allImportScopesAtInceptionYear
        )
        ...
        if allImportScopesNotAtInceptionYear:
            ...
            temp = [
                ...
                for p in self.database.Query(PartitionByReportingNodeAndPeriod)
                if (
                    p.ReportingNode == self.args.ReportingNode
                    and p.Year == self.PreviousReportingPeriod.Year
                    and p.Month == self.PreviousReportingPeriod.Month
                    and not p.Scenario
                )
            ]
            ...
            self.database.Partition.Set(
                PartitionByReportingNodeAndPeriod, self.PreviousPeriodPartition
            )
            ...
            temp = [
                rv
                for rv in self.database.Query(RawVariable)
                if rv.Partition == self.PreviousPeriodPartition
                and rv.AocType == AocTypes.CL
            ]
            ...
            temp = [
                iv
                for iv in self.database.Query(IfrsVariable)
                if iv.Partition == self.PreviousPeriodPartition
                and iv.AocType == AocTypes.EOP
            ]
        ...
        primaryScopeAtInceptionYear = {
            dn for dn in allImportScopesAtInceptionYear if dn in primaryScope
        }
        secondaryScopeAtInceptionYear = {
            dn for dn in allImportScopesAtInceptionYear if dn in secondaryScope
        }
        if (
            allImportScopesAtInceptionYear
            and self.ImportFormat != ImportFormats.Opening
        ):
            ...
            temp = [
                ...
                and iv.AocType == AocTypes.BOP
                and iv.Novelty == Novelties.I
            ]
            temp = [
                iv
                for iv in temp
                if iv.DataNode in primaryScopeAtInceptionYear
        ...
        rawVariables = (
            ...
            + [
                rv
                for rv in self.database.Query(RawVariable)
                if rv.Partition == self.TargetPartition
                and rv.DataNode in primaryScopeFromLinkedReinsurance
            ]
        )
        ...
        ifrsVariables = (
            ...
            + [
                ...
                and (
                    ...
                    and not iv.EstimateType
                    in self.EstimateTypesByImportFormat[self.ImportFormat]
        ...
        if self.DefaultPartition != self.TargetPartition:
            ...
            self.database.Partition.Set(
                PartitionByReportingNodeAndPeriod, self.DefaultPartition
            )
            defaultRawVariables = [
                rv
                for rv in self.database.Query(RawVariable)
                if rv.Partition == self.DefaultPartition and rv.DataNode in primaryScope
            ]
            defaultIfrsVariables = [
                iv
                for iv in self.database.Query(IfrsVariable)
                if iv.Partition == self.DefaultPartition
                and (
                    iv.DataNode in primaryScopeFromParsedVariables
                    and not iv.EstimateType
                    in self.EstimateTypesByImportFormat[self.ImportFormat]
            ...
            def _rawVariablesCompare(
                x: RawVariable, y: RawVariable, ignoreValues: bool = False
            ):
                if (
                    x.AccidentYear == y.AccidentYear
                    and x.AmountType == y.AmountType
                    and x.DataNode == y.DataNode
                    and x.AocType == y.AocType
                    and x.Novelty == y.Novelty
                    and x.EstimateType == y.EstimateType
                ):
                    if ignoreValues:
                        return True
                    else:
            ...
            def _ifrsVariablesCompare(
                x: IfrsVariable, y: IfrsVariable, ignoreValues: bool = False
            ):
                if (
                    x.AccidentYear == y.AccidentYear
                    and x.AmountType == y.AmountType
    ...
    def GetValuationPeriod(self, id_: ImportIdentity) -> ValuationPeriod:
        return self.AocConfigurationByAocStep[
    ...
    def GetCalculatedTelescopicAocSteps(self) -> list[AocStep]:
        return [
            k
            for k, v in self.AocConfigurationByAocStep.items()
            if v.DataType == DataType.CalculatedTelescopic
        ]

    # YieldCurve

    def GetYearlyYieldCurve(
        self, id_: ImportIdentity, economicBasis: str
    ) -> list[float]:
        yc = self.GetYieldCurve(id_, economicBasis)
        return yc.Values[
            self.args.Year - yc.Year :
        ]  # Check if the returned array is empty? Log Warning?

    def GetYieldCurve(self, id_: ImportIdentity, economicBasis: str) -> YieldCurve:

        if (economicBasis, self.GetYieldCurvePeriod(id_)) == (
            EconomicBases.C,
            PeriodType.BeginningOfPeriod,
        ):
            return self.CurrentYieldCurve[
                self.DataNodeDataBySystemName[id_.DataNode].ContractualCurrency
            ][PreviousPeriod]
        elif (economicBasis, self.GetYieldCurvePeriod(id_)) == (
            EconomicBases.C,
    ...
    def GetIfrsVariables(self, dataNode: str) -> list[IfrsVariable]:
        return list(
            v
            for v in self.IfrsVariablesByImportIdentity.get(dataNode, [])
            if v.Partition == self.TargetPartition
        )
    ...
    def GetIfrsVariable(
        ...
        vars = list(
            v
            for v in self.database.Query(IfrsVariable, include_sub=False)
            if (
                v.DataNode == id_.DataNode
                and v.Partition == self.TargetPartition
                and v.AocType == id_.AocType
                and v.Novelty == id_.Novelty
                and v.AccidentYear == accidentYear
                and v.AmountType == amountType
                and v.EstimateType == estimateType
                and v.EconomicBasis == economicBasis
            )
        )
    ...
    def GetValues(
        self, id_: ImportIdentity, whereClause: Callable[[RawVariable], bool]
    ) -> list[float]:
    ...
    def GetValues2(
        self, id_: ImportIdentity, amountType: str, estimateType: str, accidentYear: int
    ) -> list[float]:
        return self.GetValues(
            id_,
            lambda v: (
                v.AccidentYear == accidentYear
                and v.AmountType == amountType
                and v.EstimateType == estimateType
            ),
        )
    ...
    def GetValue(
        ...
        *,
        ...
        if isinstance(amountTypeORwhereClause, str) or amountTypeORwhereClause is None:
            ...
            vals = [
                v.Value
                for v in self.GetIfrsVariables(id.DataNode)
                if (v.AocType, v.Novelty) == id.AocStep and whereClause(v)
            ]
            return vals[-1] if vals else 0  # Aggregate() returns last element

    # Novelty

    def GetNoveltiesForAocType(
        self, aocType: str, aocConfiguration: set[AocStep]
    ) -> list[str]:
        return [
            aocStep.Novelty
            for aocStep in aocConfiguration
            if aocStep.AocType == aocType
        ]
    ...
    def GetNovelties2(self, aocType: str) -> list[str]:
        return self.GetNoveltiesForAocType(
            aocType, set(self.AocConfigurationByAocStep.keys())
        )

    def GetNovelties3(self, aocType: str, inputSource: InputSource) -> list[str]:
        return self.GetNoveltiesForAocType(
            aocType, self.aocStepByInputSource[inputSource]
        )
    ...
    def GetNonPerformanceRiskRate(self, identity: ImportIdentity) -> float:
        ...
        period = (
            PreviousPeriod
            if self.GetCreditDefaultRiskPeriod(identity) == PeriodType.BeginningOfPeriod
            else CurrentPeriod
        )
    ...
    def GetUnderlyingGic(self, id_: ImportIdentity) -> list[str]:
        ...
        if not interDataNodeParameters:
            ...
            temp = [
                x.DataNode if x.DataNode != id_.DataNode else x.LinkedDataNode
                for x in interDataNodeParameters[CurrentPeriod]
            ]
            return [
                goc
                for goc in temp
    ...
    def GetReinsuranceCoverage(self, id_: ImportIdentity, gic: str) -> float:
        ...
        targetPeriod = (
            CurrentPeriod
            if self.AocConfigurationByAocStep[
                AocStep(id_.AocType, id_.Novelty)
            ].RcPeriod
            == PeriodType.EndOfPeriod
            else PreviousPeriod
        ...
        interDataNodeParameters = self.InterDataNodeParametersByGoc.get(
            id_.DataNode, None
        )
        ...
        if interDataNodeParameters:
            return next(
                x
                for x in interDataNodeParameters[targetPeriod]
                if (x.DataNode == gic or x.LinkedDataNode == gic)
            ).ReinsuranceCoverage
        else:
