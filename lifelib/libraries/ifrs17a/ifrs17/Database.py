...
class IfrsDatabase(BaseDatabase):
    ...
    def DefineFormat(
        ...
        body: Callable[["IfrsDatabase", IDataSet], ImportArgs],
    ):
    ...
    def ImportFile(
        ...
        if type_:
            ...
            for name in opyxl.load_workbook(pathToFile).sheetnames:
                sheets[name] = df = replace_nan(
                    pd.read_excel(pathToFile, sheet_name=name)
                )

            return self._import_formats[format_](self, IDataSet(sheets))

    @staticmethod
    def _FromFileWithType(
        pathToFile: str, type_: Optional[Union[type, list[type]]] = None
    ):
    ...
    def FromDataSet(
        self, dataSet: IDataSet, type_: type, body: Callable = None, format_=None
    ):
        ...
        if body:
            ...
            self.Update(
                type_, self._df_to_records(dataSet.Tables[type_.__name__], type_)
            )
    ...
    def LoadCurrentParameter(
        ...
        filterExpression: Callable[[IWithYearAndMonth], bool] = None,
    ) -> dict[str, IWithYearAndMonth]:

        temp = [
            x
            for x in self.LoadParameter(type_, args.Year, args.Month, filterExpression)
        ...
        return {
            k: max(v, key=lambda y: y.Year * 100 + y.Month, default=None)
    ...
    def LoadCurrentAndPreviousParameter(
        ...
        filterExpression: Optional[Callable[[IWithYearMonthAndScenario], bool]] = None,
    ) -> dict[str, dict[int, IWithYearMonthAndScenario]]:

        parameters = {}
        for x in [
            yc
            for yc in self.LoadParameter(type_, args.Year, args.Month, filterExpression)
        ...
        for k, p in parameters.items():
            ...
            currentCandidate = max(
                [x for x in p if x.Year == args.Year],
                default=None,
                key=lambda y: y.Year * 100 + y.Month,
            )
            previousCandidate = max(
                [x for x in p if x.Year < args.Year and not x.Scenario],
                default=None,
                key=lambda y: y.Year * 100 + y.Month,
            )
            currentCandidateBE = max(
                [x for x in p if x.Year <= args.Year and not x.Scenario],
                default=None,
                key=lambda y: y.Year * 100 + y.Month,
            )
            ...
            inner[CurrentPeriod] = (
                currentCandidate if currentCandidate else previousCandidate
            )
            inner[PreviousPeriod] = (
                previousCandidate
                if previousCandidate
                else currentCandidateBE if currentCandidateBE else currentCandidate
            )
    ...
    # Yield Curve
    def LoadLockedInYieldCurve(
        self, args: Args, dataNodes: list[DataNodeData]
    ) -> dict[str, YieldCurve]:

        lockedInYieldCurveByGoc: dict[str, YieldCurve] = dict()

        for dn in [
            x for x in dataNodes if x.ValuationApproach == ValuationApproaches.BBA
        ]:

            argsNew = dataclasses.replace(
                args, Year=dn.Year, Month=dn.Month, Scenario=dn.Scenario
            )

            loadedYc = self.LoadCurrentParameter(
                YieldCurve,
                argsNew,
                lambda x: x.Currency,
                lambda x: x.Currency == dn.ContractualCurrency,
            )
    ...
    def LoadCurrentYieldCurve(
        self, args: Args, dataNodes: list[DataNodeData]
    ) -> dict[str, dict[int, YieldCurve]]:
        contractualCurrenciesInScope = {dn.ContractualCurrency for dn in dataNodes}
        return self.LoadCurrentAndPreviousParameter(
            YieldCurve,
            args,
            lambda x: x.Currency,
    ...
    def LoadDataNodes(self, args: Args) -> dict[str, DataNodeData]:
        ...
        dataNodeStates = self.LoadCurrentAndPreviousParameter(
            DataNodeState, args, lambda x: x.DataNode
        )
        activeDataNodes = [
            k
            for k, v in dataNodeStates.items()
            if v[CurrentPeriod].State != State.Inactive
        ]

        temp = [
            dn for dn in self.Query(GroupOfContract) if dn.SystemName in activeDataNodes
        ]
        result = {}
        for dn in temp:
            dnCurrentState = dataNodeStates[dn.SystemName][CurrentPeriod]
            dnPreviousState = dataNodeStates[dn.SystemName][PreviousPeriod]
            result[dn.SystemName] = DataNodeData(
    ...
    def LoadParameter(
        ...
        year: int,
        month: int,
        filterExpression: Optional[Callable[[IWithYearAndMonth], bool]] = None,
    ) -> list[IWithYearAndMonth]:

        result = [
            x
            for x in self.Query(type_)
            if x.Year == year and x.Month <= month or x.Year < year
        ]
    ...
    def LoadSingleDataNodeParameters(
        self, args: Args
    ) -> dict[str, dict[int, SingleDataNodeParameter]]:
        return self.LoadCurrentAndPreviousParameter(
            SingleDataNodeParameter, args, lambda x: x.DataNode
        )

    def LoadInterDataNodeParameters(
        self, args: Args
    ) -> dict[str, dict[int, set[InterDataNodeParameter]]]:

        identityExpressions: list[Callable[[InterDataNodeParameter], str]] = [
            lambda x: x.DataNode,
            lambda x: x.LinkedDataNode,
        ]
        parameterArray = self.LoadParameter(
            InterDataNodeParameter, args.Year, args.Month
        )
        ...
        for ie in identityExpressions:
            ...
            for p in parameterArray:
                ...
                if k in temp:
                    ...
                    temp[k] = [p]
            parameters.extend(
                [IGroupingParams(Key=key, Values=val) for key, val in temp.items()]
            )

        def _inner(p, gg):
            currentCandidate = max(
                [x for x in gg if x.Year == args.Year],
            ...
            return TempName1(
                ...
                previousPeriod=(
                    previousCandidate if previousCandidate else currentCandidate
                ),
            )
        ...
        for k, v in temp.items():
            temp2[k] = {
                CurrentPeriod: set(y.currentPeriod for y in v),
    ...
    def LoadAocStepConfigurationAsDictionary(
        self, year: int, month: int
    ) -> dict[AocStep, AocConfiguration]:
        return {
            AocStep(x.AocType, x.Novelty): x
            for x in self.LoadAocStepConfiguration(year, month)
        }

    # Import helper

    def GetArgsFromMain(
        self, PartitionType: IKeyedType, dataSet: IDataSet
    ) -> ImportArgs:
        ...
        scenario = (
            main["Scenario"]
            if "Scenario" in mainTab.columns and main["Scenario"]
            else ""
        )  # Convert nan to ''
        ...
        if PartitionType is PartitionByReportingNode:
            ...
            self.Update(
                ...
                [
                    PartitionByReportingNode(
                        Id=self.Partition.GetKeyForInstance(
                            PartitionByReportingNode, args
                        ),
            ...
            self.Update(
                ...
                [
                    PartitionByReportingNodeAndPeriod(
                        Id=self.Partition.GetKeyForInstance(
                            PartitionByReportingNodeAndPeriod, args
                        ),
                        Year=args.Year,
                        Month=args.Month,
                        ReportingNode=reportingNode,
    ...
    def DataNodeFactory(self, dataSet: IDataSet, tableName: str, args: ImportArgs):
        partition = [
            p
            for p in self.Query(PartitionByReportingNode)
        ...
        dataNodesDefined = [
            x for x in self.Query(GroupOfContract) if x.SystemName in dataNodesImported
        ]
        dataNodeStatesDefined = [x.DataNode for x in self.Query(DataNodeState)]
        dataNodeParametersDefined = [
            x.DataNode for x in self.Query(SingleDataNodeParameter)
        ]

        dataNodeStatesUndefined = remove_duplicates(
            [x for x in dataNodesImported if x and not x in dataNodeStatesDefined]
        )
        dataNodeSingleParametersUndefined = remove_duplicates(
            [
                x
                for x in dataNodesImported
                if (
                    x
                    and not x in dataNodeParametersDefined
                    and isinstance(
                        [y for y in dataNodesDefined if y.SystemName == x][0],
    ...
    def _add_goc_attrs(self, data: pd.DataFrame):
        ...
        df = pd.merge(
            data, goc, left_on="DataNode", right_index=True, how="left", sort=False
        )
