...
print("planned premium {:,.2f}/yr {} mode, load {:.0%}   indexed allocation {:.0%}"
      "   fixed account {:.2%} (guaranteed {:.2%})".format(
          ...
          proj.index_alloc_rate(), proj.fixed_rate_ann(0), proj.guar_rate_ann))
print("index account: cap {:.2%} (guaranteed {:.2%}), par {:.0%}, floor {:.0%}"
      "   level index return {:.2%}/yr   credited rate {:.2%}".format(
          proj.index_cap_at(0), proj.index_cap_guar, proj.index_par,
...
print()
print("Account value roll-forward (per policy) - policy months t = 0-13")
print(proj.result_av().head(14).round(2).to_string())
print()
print("Indexed segment ladder - policy months t = 0-13")
print(proj.result_seg().head(14).round(2).to_string())
print()
print("Liability cash flows - policy months t = 0-13")
