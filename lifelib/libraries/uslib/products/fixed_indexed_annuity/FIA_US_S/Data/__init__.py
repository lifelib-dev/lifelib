...
and the MVA reference yield ``i_t``; there ``t`` is an **anniversary** number, 0 at issue,
and it is **not** the projection's month index. The projection differences consecutive
levels into ``R(t) = I(y+1)/I(y) - 1`` with ``y = duration(t)``, the return over the
contract year containing month ``t``, so
holding a level flat is how a scenario says "no index credit this year".
``withdrawal_table`` is indexed by ``(wd_schedule_id, t)``, there an **elapsed contract
year** count rather than the projection's month index, and is *not* a step function — a
contract year with no row simply takes no ad hoc withdrawal.
...
def rate_scenario():
    ...
    Indexed by ``(scenario_id, t)``, where ``t`` is an **anniversary** number with 0 at
    issue and is **not** the projection's month index: anniversary ``y`` opens the contract
    year ``y + 1``, so the file's keys are already 0-based and are read unshifted at
    ``duration(t)``.  A step function: each row states the index level and the MVA reference
    yield that hold from that anniversary until the next row of the same scenario.
    """
    return pd.read_csv(                                              # noqa: F821
...
def withdrawal_table():
    ...
    Indexed by ``(wd_schedule_id, t)``, where ``t`` is an **elapsed contract year** count
    rather than the projection's month index: a row keyed ``y`` is withdrawn at the
    anniversary closing contract year ``y + 1``, read at ``duration(t)``.  A contract year
    with no row takes no ad hoc withdrawal.  Lifetime withdrawals are *not* scheduled here —
    they are generated by the rider from ``utilization_intensity`` and the locked payout
    percentage.
    """
    return pd.read_csv(                                              # noqa: F821
