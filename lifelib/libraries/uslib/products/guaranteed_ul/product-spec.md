...
guideline premium test [S2, S4 level-only design; R4 corridor requirement](#uslib-guaranteed_ul-s2). Because
...
[S4; S2, S9](#uslib-guaranteed_ul-s4).
...
the catch-up shortfall [S7; R1 example 7; formula **[std]**](#uslib-guaranteed_ul-s7).
...
1. **Guarantee mechanism.** Shadow accounts dominate flagship lifetime GUL
   ...
   indebtedness ≥ required accumulated premium schedule` [S4, S5; R1 Design #2](#uslib-guaranteed_ul-s4), and
...
4. **Missed premiums.** Some designs shorten the guarantee age gracefully (guarantee
   ...
   provision restores the original horizon [S7; R1](#uslib-guaranteed_ul-s7).
