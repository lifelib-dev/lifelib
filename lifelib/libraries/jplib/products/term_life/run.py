"""Run the Term_JP_S reference model and print its cash flow statement.
...
model = mx.read_model(Path(__file__).parent / "Term_JP_S")
point_id = int(sys.argv[1]) if len(sys.argv) > 1 else 1

proj = model.Projection[point_id]
shape = "nen manryo (renewable)" if proj.term_type() == "nen" else "sai manryo (fixed)"
print("Term_JP_S - teiki hoken (level term life), JPY, monthly grid")
...
print("premium = {:,.0f}/month ({:,.0f} p.a.), paid every {} month(s)".format(
    proj.premium_mth_pp(0), proj.prem_pp(0), proj.prem_mode_months()))
print("horizon = {} months ({} years) to attained age {}   boundary = {}".format(
    proj.proj_len(), proj.proj_len() // 12,
    proj.age_at_entry() + proj.proj_len() // 12, proj.contract_boundary()))
print("modules: living_needs = {}   wop = {}   reinstatement = {}".format(
    proj.living_needs(), proj.wop(), proj.reinstatement()))
print()

df = proj.result_cf()
print("cash flow statement, t = 0 .. {} (t is 0-based and counts policy MONTHS; "
      "policy year = t // 12 + 1)".format(proj.proj_len() - 1))
print(df.head(13).round(2).to_string())
if len(df) > 13:
    print("... {} further months to t = {}".format(len(df) - 13, proj.proj_len() - 1))
print()
annual = df.groupby(df.index // 12).sum()
annual["pols_if"] = [proj.pols_if(12 * y) for y in annual.index]
annual.index.name = "policy year - 1"
print("the same statement grouped into policy years (pols_if read at the anniversary):")
print(annual.head(12).round(2).to_string())
print()
