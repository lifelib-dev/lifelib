...
The time index is the library's 0-based policy month: ``t = 0`` is the first month, so
the notes' first worked-example row is ``av_pp(0)`` and their twelfth month is
``t = 11``; the frame is ``range(proj_len())``, ``t = 0 .. proj_len() - 1``.  ``av_pp(t)``
is the notes' ``UF(t)``, the fund at the start of month t, and ``av_pp_at(t, "AFT_WD")``
is ``UF(t+1)``.
...
# t: (UF(t), gross return, tax provision, AMC, further costs, withdrawal, UF(t+1)),
# t the 0-based policy month; t = 11 is the twelfth month, the last of policy year 1.
WORKED_EXAMPLE = {
    0:  (100000.00, 407.41, 81.48, 83.60, 8.36, 416.67, 99817.30),
    1:  (99817.30,  406.67, 81.33, 83.45, 8.35, 416.67, 99634.17),
    2:  (99634.17,  405.92, 81.18, 83.30, 8.33, 416.67, 99450.61),
    11: (97966.60,  399.13, 79.83, 81.90, 8.19, 416.67, 97779.14),
}
...
def test_worked_example_month_one_trace(uk_bond_anchor):
    ...
    assert p.av_pp_at(0, "AFT_GROWTH") == pytest.approx(100325.93, abs=PENNY)
    assert p.av_pp_at(0, "AFT_CHARGE") == pytest.approx(100233.96, abs=PENNY)
    ...
    assert p.wd_pp(0) == pytest.approx(5000.0 / 12, rel=1e-14)
    assert p.av_pp_at(0, "AFT_WD") == pytest.approx(99817.30, abs=PENNY)


def test_worked_example_year_one_totals(uk_bond_anchor):
    """The notes' totals row, and the reconciliation that closes it."""
    p = uk_bond_anchor
    months = range(12)                     # policy year 1 is t = 0 .. 11
    ...
    assert p.av_pp_at(11, "AFT_WD") == pytest.approx(YEAR_1["uf_end"], abs=PENNY)
    assert p.av_pp(12) == pytest.approx(YEAR_1["uf_end"], abs=PENNY)   # UF(12)
...
def test_worked_example_per_segment(uk_bond_anchor):
    ...
    assert p.av_per_segment_pp(0) == pytest.approx(1000.00, abs=PENNY)
    assert p.av_per_segment_pp(12) == pytest.approx(977.79, abs=PENNY)
...
def test_worked_example_insurer_side(uk_bond_anchor):
    ...
    # Maintenance is GBP 60 in year 1, and the GBP 300 acquisition charge falls at
    # issue - in the first month, t = 0, and in no other.
    assert p.expenses(0) - 300.0 * p.pols_if(0) == pytest.approx(5.00, abs=PENNY)
    assert p.expenses(1) == pytest.approx(5.00 * p.pols_if(1), abs=PENNY)
    ...
    assert p.mort_rate(0) == pytest.approx(0.01, rel=1e-12)
    assert p.mort_rate_mth(0) == pytest.approx(Q_M, abs=5e-7)


def test_worked_example_death_benefit_at_month_twelve(uk_bond_anchor):
    """Per actual death in the twelfth month, t = 11, the benefit is 1.001 x 97,779.14
    = 97,876.92 and the strain 97.78."""
    p = uk_bond_anchor
    uf = p.av_pp_at(11, "AFT_WD")
    assert 1.001 * uf == pytest.approx(97876.92, abs=PENNY)
    assert p.death_strain_pp(11) == pytest.approx(97.78, abs=PENNY)
    assert p.claims(11, "DEATH") == pytest.approx(
        97876.92 * p.pols_death(11), abs=PENNY)
...
def test_worked_example_allowance_check(uk_bond_anchor):
    ...
    p = uk_bond_anchor
    assert p.wd_cum_pp(11) == pytest.approx(5000.00, abs=PENNY)
    assert p.allowance_cum_pp(11) == pytest.approx(5000.00, abs=PENNY)
    assert p.excess_gain_pp(11) == pytest.approx(0.0, abs=PENNY)
    # And the 7.5% product cap - 7,500 on the premium - is nowhere near breached.
    assert p.wd_cap_pp(0) == pytest.approx(0.075 * 100233.96, abs=PENNY)
    assert 12 * p.wd_pp(0) < p.wd_cap_pp(0)
...
def test_the_amc_is_levied_on_the_post_growth_pre_cancellation_fund(uk_bond_anchor):
    """Not the opening fund UF(t), and not the post-withdrawal fund - a listed pitfall.
...
def test_the_unit_fund_rollforward_closes(unit_linked_bond):
    """UF(t+1) = UF(t)(1 + g_m(1-t_pf))(1 - c_m - f_m) - W - AC - GC, from UF(0) = P."""
    ...
    assert p.av_pp(0) == p.av_init_pp() == p.premium()
    for t in (0, 11, 99):
...
def test_the_timing_strings_validate(uk_bond_anchor):
    with pytest.raises(FormulaError):
        uk_bond_anchor.av_pp_at(0, "AFT_NOTHING")
    with pytest.raises(FormulaError):
        uk_bond_anchor.pols_if_at(0, "BEF_NOTHING")
...
def test_adviser_charges_are_a_pass_through(unit_linked_bond):
    ...
    assert p3.adviser_charge_pp(0) == pytest.approx(
        0.005 / 12 * p3.av_pp_at(0, "AFT_CHARGE"), rel=1e-14)
    # It draws the fund down faster, so the fund is exhausted sooner.
    assert p3.av_pp(59) < p1.av_pp(59)
    assert p3.proj_len() < p1.proj_len()
    # And it consumes the tax allowance alongside the withdrawal.
    assert p3.wd_cum_pp(11) > p1.wd_cum_pp(11)
    # But it appears in no income line: net_cf is still AMC less expenses and strain.
    assert p3.net_cf(11) == pytest.approx(
        p3.amc_charges(11) - p3.expenses(11) - p3.death_strain(11), rel=1e-12)
...
def test_the_death_strain_is_the_uplift(uk_bond_anchor):
    """(u - 1) x UF(t+1): a tenth of a percent of the end-of-month fund, and nothing else
    in the base run."""
    p = uk_bond_anchor
...
def test_the_guarantee_erodes_with_withdrawals(unit_linked_bond):
    ...
    On the anchor withdrawal pattern the guarantee reaches zero at t = 240, once twenty
    years of withdrawals (t = 0 .. 239) have been drawn - which is why the rider never
    bites on the base assumptions, and why model point 4's cash flows are all but
    identical to point 1's.
    """
    p = unit_linked_bond.Projection[4]
    assert p.gmdb_flag() is True
    assert p.gmdb_guarantee_pp(0) == pytest.approx(100000.0, abs=PENNY)   # nothing drawn
    # Measured before the month's own cancellation: 119 withdrawals, t = 0 .. 118.
    assert p.gmdb_guarantee_pp(119) == pytest.approx(
        100000.0 - 119 * 416.6667, abs=0.02)
    assert p.gmdb_guarantee_pp(239) > 0.0
    assert p.gmdb_guarantee_pp(240) == 0.0
    # Out of the money throughout, so the charge is zero and the strain is the uplift.
    assert all(p.gmdb_charge_pp(t) == 0.0 for t in (0, 59, 119, 199))
    assert p.death_strain_pp(59) == pytest.approx(
        0.001 * p.av_pp_at(59, "AFT_WD"), rel=1e-12)
...
def test_the_guarantee_bites_when_the_fund_falls(unit_linked_bond):
    ...
    try:
        ...
        assert p.gmdb_guarantee_pp(59) > p.db_uplift() * p.av_pp_at(59, "AFT_CHARGE")
        assert p.gmdb_charge_pp(59) > 0.0
        assert p.death_strain_pp(59) > 0.001 * p.av_pp_at(59, "AFT_WD")
        assert p.gmdb_charges(59) > 0.0          # the charge is insurer income
        assert p.check_unit_funding() is True
        # The death benefit is the guarantee where it exceeds the uplifted fund.
        assert p.claims(59, "DEATH") == pytest.approx(
            p.gmdb_guarantee_pp(59) * p.pols_death(59), rel=1e-12)
    finally:
        model.close()
...
def test_the_guarantee_is_measured_before_the_months_cancellations(unit_linked_bond):
    ...
    depends on the charge.  In the first month nothing has been drawn.
    """
    p = unit_linked_bond.Projection[4]
    for t in (1, 59, 119):
        assert p.gmdb_guarantee_pp(t) == pytest.approx(
            max(0.0, p.premium() - p.wd_cum_pp(t - 1)), abs=1e-9)
    assert p.gmdb_guarantee_pp(0) == p.premium()
    assert p.wd_cum_pp(0) == pytest.approx(p.wd_pp(0) + p.adviser_charge_pp(0))
...
def test_the_withdrawal_patterns(unit_linked_bond):
    ...
    assert p1.wd_pp(0) == pytest.approx(0.05 * 100000 / 12, rel=1e-14)

    p2 = unit_linked_bond.Projection[2]
    assert p2.wd_pattern() == "none" and p2.wd_rate() == 0.0
    assert all(p2.wd_pp(t) == 0.0 for t in (0, 59, 399))

    p6 = unit_linked_bond.Projection[6]
    assert p6.wd_pattern() == "custom" and p6.wd_rate() == 0.03
    assert p6.wd_pp(0) == pytest.approx(0.03 * 250000 / 12, rel=1e-14)
...
def test_the_product_cap_is_the_seven_and_a_half_percent_one(uk_bond_anchor):
    ...
    """
    p = uk_bond_anchor
    assert p.wd_cap_pp(0) == pytest.approx(
        0.075 * max(p.av_pp_at(0, "AFT_CHARGE"), 100000.0), abs=PENNY)
...
def test_the_withdrawal_is_capped_at_what_the_fund_can_pay(uk_bond_anchor):
    ...
    The last projected month, t = proj_len() - 1, opens with units and closes with none.
    """
    p = uk_bond_anchor
    last = p.proj_len() - 1
    assert p.av_pp(last) > 0.0
    assert p.av_pp_at(last, "AFT_WD") == pytest.approx(0.0, abs=1e-9)
    assert p.av_pp(last + 1) == pytest.approx(0.0, abs=1e-9)
    assert p.wd_pp(last) <= p.av_pp_at(last, "AFT_CHARGE") + 1e-9
    assert all(p.av_pp(t) >= 0.0 for t in range(p.proj_len()))
...
def test_the_fund_exhausts_the_projection(unit_linked_bond):
    ...
    A bond with no units has no liability, no margin and nothing left to project.  The
    anchor's fund is drawn to nothing at the end of month t = 352: fund_exhaust_mth() is
    the time point 353 at which av_pp reaches zero, and proj_len() the 353 months
    t = 0 .. 352.
    """
    p1, p2 = unit_linked_bond.Projection[1], unit_linked_bond.Projection[2]
    assert p1.fund_exhaust_mth() == 353
    assert p1.av_pp(352) > 0.0 and p1.av_pp(353) == 0.0
    assert p1.proj_len() == 353 < p1.horizon_mths()
    assert p1.proj_len() == p1.fund_exhaust_mth()
    ...
    assert p2.av_pp(p2.proj_len() - 1) > p2.premium()  # and its fund has grown
...
def test_the_allowance_accrues_for_twenty_years_then_stops(uk_bond_anchor):
    ...
    p = uk_bond_anchor
    assert p.allowance_cum_pp(11) == pytest.approx(5000.0, abs=PENNY)     # year 1
    assert p.allowance_cum_pp(239) == pytest.approx(100000.0, abs=PENNY)  # year 20
    assert p.allowance_cum_pp(251) == pytest.approx(100000.0, abs=PENNY)  # year 21
    assert p.allowance_cum_pp(299) == pytest.approx(100000.0, abs=PENNY)
...
def test_the_excess_gain_appears_once_the_allowance_is_spent(uk_bond_anchor):
    ...
    p = uk_bond_anchor
    assert p.excess_gain_pp(239) == pytest.approx(0.0, abs=PENNY)   # end of year 20
    assert p.excess_gain_pp(251) > 0.0
    assert p.excess_gain_pp(299) > p.excess_gain_pp(251)


def test_the_allowance_exhaustion_step_raises_surrender(uk_bond_anchor):
    """M_allow steps to 1.5 from policy year 21, which opens at t = 240."""
    p = uk_bond_anchor
    assert p.policy_year(0) == 1 and p.duration(0) == 0
    assert p.policy_year(239) == 20 and p.policy_year(240) == 21
    assert p.allow_factor(239) == 1.0
    assert p.allow_factor(240) == 1.5
    assert p.surr_rate(240) == pytest.approx(
        min(0.35, p.surr_rate_base(240) * 1.5), rel=1e-12)
...
def test_the_performance_multiplier_is_off_and_bites_when_switched_on(unit_linked_bond):
    ...
    """
    p = unit_linked_bond.Projection[1]
    assert p.return_12m(0) == pytest.approx(0.05, rel=1e-12)   # equals the reference
    assert all(p.perf_factor(t) == 1.0 for t in (0, 59, 199))
    ...
    try:
        ...
        assert proj.return_12m(0) == pytest.approx(0.05 - 0.20, rel=1e-12)
        assert proj.perf_factor(0) == pytest.approx(min(2.0, 1 + 2 * 0.20), rel=1e-12)
        assert proj.surr_rate(0) > p.surr_rate(0)
        # But the fund path is untouched, which is the point of the separate Reference.
        assert proj.av_pp(59) == pytest.approx(p.av_pp(59), rel=1e-12)
    finally:
        model.close()
...
def test_the_surrender_rate_is_capped(unit_linked_bond):
    ...
    try:
        ...
        model.Projection.clear_all()
        proj = model.Projection[1]
        assert proj.perf_factor(0) == 2.0         # the multiplier caps first
        assert proj.surr_rate(299) == pytest.approx(     # policy year 25: ultimate 10%
...
def test_a_surrender_costs_nothing_at_exit_but_truncates_the_margin(uk_bond_anchor):
    ...
    assert p.pols_if(199) < p.pols_if(11)


def test_the_inforce_rollforward_closes(unit_linked_bond):
    """l(0) = pols_if_init, and the survivors of the last month t = proj_len() - 1 go
    to pols_maturity, so pols_if(proj_len()) is zero."""
    for point_id in unit_linked_bond.Data.model_point_table().index:
    ...
    assert all(p.pols_maturity(t) == 0.0 for t in (0, 99, last - 1))
    assert p.pols_maturity(last) > 0.0
    assert p.pols_if_at(last, "AFT_DECR") == 0.0 and p.pols_if(last + 1) == 0.0
...
def test_result_cf_shape(uk_bond_anchor):
    """One row per projected month, t = 0 .. proj_len() - 1; 353 rows on the anchor."""
    p = uk_bond_anchor
    df = p.result_cf()
    assert df.index.name == "t"
    assert list(df.index) == list(range(353)) == list(range(p.proj_len()))
...
def test_result_uf_matches_the_notes_table(uk_bond_anchor):
    """result_uf is the notes' worked-example table, column for column, row t = 0 first."""
    df = uk_bond_anchor.result_uf()
    for name in ("av_pp", "fund_growth_pp", "tax_provision_pp", "amc_pp",
                 "further_costs_pp", "wd_pp", "av_pp_end"):
        assert name in df.columns
    assert list(df.index) == list(range(uk_bond_anchor.proj_len()))
    assert df.loc[0, "av_pp"] == pytest.approx(100000.00, abs=PENNY)
    assert df.loc[0, "av_pp_end"] == pytest.approx(99817.30, abs=PENNY)
    assert df.loc[11, "av_pp_end"] == pytest.approx(97779.14, abs=PENNY)
