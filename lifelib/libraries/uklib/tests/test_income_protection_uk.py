...
discounted at a flat 3%.  Model point 2 is that cell.  The notes also give the first-month
active-lives figures beside it, and model point 1 is that cell.

The policy month ``t`` is 0-based: ``t = 0`` is the first projected month, the frame is
``range(proj_len())`` and the worked example's three rows are ``t = 0, 1, 2``.  The claim
duration ``z`` is a separate 1-based cohort index and does not shift with ``t``.
...
# t: (l_S at start, recoveries, deaths in claim, l_S at end, benefit, v(t) = v^(t+1), PV)
WORKED_EXAMPLE = {
...
V_M = 0.997540            # 1.03^(-1/12), one month's discount: disc_factor(0)
...
def test_worked_example_row(uk_ip_claim, t):
    """Every cell of the notes' three-month table, to the displayed precision.

    Row ``t`` is month ``t`` of the frame: the population at its start, the exits and
    the benefit during it, and the discount factor to its end, time ``t + 1``.
    """
    start, rec, dth, end, ben, v, pv = WORKED_EXAMPLE[t]
    p = uk_ip_claim
    assert p.pols_sick(t) == pytest.approx(start, abs=STATE)
    assert p.pols_recovery(t) == pytest.approx(rec, abs=STATE)
    assert p.pols_death_sick(t) == pytest.approx(dth, abs=STATE)
    assert p.pols_sick_surv(t) == pytest.approx(end, abs=STATE)
    assert p.claims(t, "BENEFIT") == pytest.approx(ben, abs=PENNY)
    assert p.disc_factor(t) == pytest.approx(v, abs=5e-7)
    assert p.claims(t, "BENEFIT") * p.disc_factor(t) == pytest.approx(pv, abs=PENNY)


def test_worked_example_three_month_pv(uk_ip_claim):
    """The notes' headline: GBP 5,459.59 per claim in payment over three months."""
    p = uk_ip_claim
    total = sum(p.claims(t, "BENEFIT") * p.disc_factor(t) for t in (0, 1, 2))
...
def test_worked_example_monthly_factors(uk_ip_claim):
    ...
    # v(t) discounts to the end of month t, so the first month's factor is v^1.
    assert p.disc_factor(0) == pytest.approx(V_M, abs=5e-7)
    assert p.disc_factor(1) == pytest.approx(V_M ** 2, abs=1e-6)


def test_worked_example_is_a_survival_product(uk_ip_claim):
    """l_S(t+1) = s_S^(t+1) while the duration-year-1 rates hold: exits leave for good."""
    p = uk_ip_claim
    s = p.claim_surv_step(1)
    assert p.pols_sick(0) == 1.0
    for t in (0, 1, 2, 11):
        assert p.pols_sick_surv(t) == pytest.approx(s ** (t + 1), rel=1e-12)
        assert p.pols_sick(t + 1) == pytest.approx(s ** (t + 1), rel=1e-12)


def test_worked_example_active_side(income_protection):
    """The notes' first-month active-lives figures: premium 35.00 and n(0) = 0.000108."""
    p = income_protection.Projection[1]
    assert p.premiums(0) == pytest.approx(35.00, abs=PENNY)
    assert p.pols_active(0) == 1.0 == p.pols_if_init()
    assert p.inception_rate(0) == pytest.approx(0.0013, rel=1e-12)
    assert p.inception_rate_mth(0) == pytest.approx(0.000108, abs=5e-7)
    # n(0) is the inception rate net of the month's mortality and lapse, so a shade under.
    assert p.pols_inception(0) == pytest.approx(0.000107, abs=5e-7)
    assert p.pols_inception(0) < p.inception_rate_mth(0)
...
def test_collapsing_the_duration_dimension_would_misstate_the_runoff(uk_ip_claim):
    ...
    """
    p = uk_ip_claim
    cohort_surv = p.pols_sick(60)                    # start of month 60, i.e. 5 years
...
def test_the_cohort_vector_and_the_two_dimensional_view_agree(uk_ip_claim):
    ...
    ``t`` is the 0-based month; ``z`` is the 1-based cohort index and does not shift.
    """
    p = uk_ip_claim
    for t in (0, 1, 12, 99):
        by_cohort = sum(p.pols_sick_dur(t, z) for z in range(1, p.max_dur() + 1))
        assert p.pols_sick(t) == pytest.approx(by_cohort, rel=1e-12)
    assert p.pols_sick_dur(0, 1) == 1.0              # the seeded claim, at duration 1
    assert p.pols_sick_dur(0, 2) == 0.0
    assert p.pols_sick_dur(4, 0) == 0.0              # z out of range, not an error
    assert p.pols_sick_dur(4, 10**6) == 0.0


def test_a_seeded_claim_starts_at_its_stated_duration(income_protection):
    """Model point 5 is a claim already 30 months old, so it enters cohort 31 at t = 0."""
    p = income_protection.Projection[5]
    assert p.claim_duration_months() == 30
    assert p.pols_sick_dur(0, 31) == 1.0
    assert p.pols_sick_dur(0, 1) == 0.0
...
def test_the_cohort_vector_is_rebuilt_not_mutated(uk_ip_claim):
    ...
    p = uk_ip_claim
    v = p.sick_cohorts(2)
    before = p.pols_sick(2)
    v[0] = 999.0                                     # mutate the caller's copy
    assert p.sick_cohorts(2)[0] != 999.0 or p.pols_sick(2) == before
    assert p.pols_sick(2) == pytest.approx(before, rel=1e-14)
...
def test_a_new_inception_is_not_paid_in_the_month_it_incepts(income_protection):
    ...
    """
    p = income_protection.Projection[1]
    assert p.pols_sick(0) == 0.0                      # nobody in claim yet
    assert p.claims(0, "BENEFIT") == 0.0              # so nothing is paid
    assert p.pols_inception(0) > 0.0                  # even though claims incepted
    assert p.pols_sick_dur(1, 1) == pytest.approx(p.pols_inception(0), rel=1e-14)
    assert p.claims(1, "BENEFIT") > 0.0
...
def test_everything_terminates_at_the_policy_end_date(uk_ip_claim):
    ...
    The last projected month is ``proj_len() - 1``; nothing is in force at ``proj_len()``.
    """
    p = uk_ip_claim
    assert p.proj_len() == 360 == 12 * (p.expiry_age() - p.age_at_entry())
    assert p.pols_sick(359) > 0.0
    assert p.pols_sick(360) == 0.0
    assert p.pols_if(360) == 0.0
    assert p.pols_if_at(359, "AFT_DECR") == 0.0
    assert p.pols_if_at(358, "AFT_DECR") == p.pols_if(359)
    for t in (0, 99, 358):
        assert p.pols_maturity(t) == 0.0
    assert p.pols_maturity(359) > 0.0
...
def test_a_claim_near_expiry_is_truncated(income_protection):
    ...
    assert p.pols_sick(179) > 0.0                 # still paying in the final month
    assert p.pols_sick(180) == 0.0                # and nothing after it
...
def test_the_exit_basis_is_a_pure_disabled_life_annuity(uk_ip_claim):
    ...
    assert all(p.pols_active(t) == 0.0 for t in (0, 1, 49, 359))
    assert all(p.pols_inception(t) == 0.0 for t in (0, 1, 49, 359))
    for t in (0, 49):
        assert p.pols_exit(t) == p.pols_recovery(t)
    assert p.pols_exit_cum(0) == 0.0
    assert p.pols_exit_cum(360) > 0.0
...
def test_the_return_to_h_basis_gives_a_bigger_liability(income_protection):
    ...
    assert p7.pols_active(49) > 0.0               # recovered lives are back in H
    assert p7.pols_inception(49) > 0.0            # and exposed to inception again
    assert all(p7.pols_exit(t) == 0.0 for t in (0, 49, 199))
    # They also start paying premiums again, which the exit basis never sees.
    assert p7.premiums(49) > 0.0 and p2.premiums(49) == 0.0
...
def test_the_two_bases_agree_in_the_first_month(income_protection):
    ...
    p2, p7 = income_protection.Projection[2], income_protection.Projection[7]
    assert p2.claims(0, "BENEFIT") == pytest.approx(p7.claims(0, "BENEFIT"), rel=1e-14)
...
def test_the_rollforward_closes(income_protection):
    ...
    assert p.pols_inception(49) > 0.0 and p.pols_recovery(49) > 0.0
    assert p.pols_exit(49) == 0.0
...
def test_deaths_come_from_both_states(income_protection):
    ...
    # mort_rate is the active-life rate, by month t; the claimant rate is keyed by the
    # 1-based claim duration z.
    assert p.mort_rate(0) != pytest.approx(p.mort_rate_sick(1), rel=1e-3)
    assert p.mort_rate(0) < p.mort_rate_sick(1)      # claimants die faster
...
def test_inception_interpolation_is_linear(income_protection):
    ...
    assert p.age(0) == 35 and p.inception_rate(0) == pytest.approx(0.0013, rel=1e-12)
    assert p.age(60) == 40 and p.inception_rate(60) == pytest.approx(0.0018, rel=1e-12)
    # Inside a segment the rate is the linear reading, not the geometric one.
    t38 = 36                                         # policy year 4, attained age 38
    assert p.age(t38) == 38
...
def test_the_economic_cycle_overlay_is_off_and_moves_both_rates(income_protection):
    ...
    p = income_protection.Projection[1]
    base_inception = p.inception_rate(0)
    base_recovery = p.rec_rate(1)                     # z = 1: the first claim month
    ...
    try:
        ...
        assert proj.inception_rate(0) == pytest.approx(base_inception * 1.25, rel=1e-12)
...
def test_escalation_compounds_benefit_and_premium(income_protection):
    ...
    for y, t in ((1, 0), (2, 12), (5, 48)):         # policy year y = t // 12 + 1
    ...
    assert p.benefit_pp(11) == p.benefit_pp(0)      # steps on anniversaries only
    assert p.benefit_pp(12) > p.benefit_pp(11)      # the first at the start of t = 12
...
def test_escalation_continues_in_claim(income_protection):
    ...
    """
    p = income_protection.Projection[1]
    assert p.amount_payable_pp(12) == pytest.approx(
        p.amount_payable_pp(0) * 1.03, rel=1e-12)
    assert p.premiums(12) == pytest.approx(p.premium_pp(12) * p.pols_active(12),
                                           rel=1e-14)
...
def test_the_escalation_lapse_shock_is_off_at_a_three_percent_rpi(income_protection):
    ...
    p = income_protection.Projection[1]
    assert all(p.esc_lapse_factor(t) == 1.0 for t in (0, 12, 99))
    assert p.lapse_rate(12) == p.lapse_rate_base(12)
    ...
    try:
        ...
        model.Projection.clear_all()
        proj = model.Projection[1]
        assert proj.esc_lapse_factor(0) == 1.0    # no uplift in policy year 1
        assert proj.esc_lapse_factor(11) == 1.0   # which ends at t = 11
        assert proj.esc_lapse_factor(12) == pytest.approx(1.2, rel=1e-12)
        assert proj.lapse_rate(12) > proj.lapse_rate_base(12)
    finally:
        model.close()
...
def test_a_level_cover_cell_does_not_escalate(income_protection):
    ...
    assert all(p.benefit_pp(t) == 2000.0 for t in (0, 12, 359))
    assert all(p.premium_pp(t) == 35.0 for t in (0, 12, 359))
    assert all(p.esc_lapse_factor(t) == 1.0 for t in (0, 12, 359))
...
def test_amount_payable_never_exceeds_the_chosen_benefit(income_protection):
    ...
    p = income_protection.Projection[1]
    assert p.amount_payable_pp(0) == p.benefit_pp(0)
    ...
    try:
        ...
        model.Projection.clear_all()
        proj = model.Projection[1]
        assert proj.amount_payable_pp(0) == pytest.approx(0.85 * 2000.0, rel=1e-12)
        assert proj.amount_payable_pp(0) < proj.benefit_pp(0)
        assert proj.claims(49, "BENEFIT") < income_protection.Projection[1].claims(
            49, "BENEFIT")
    finally:
        model.close()
...
def test_invalid_enum_values_raise(uk_ip_claim):
    ...
    with pytest.raises(FormulaError):
        uk_ip_claim.pols_if_at(0, "BEF_NOTHING")
    with pytest.raises(FormulaError):
        uk_ip_claim.claims(0, "MATURITY")
...
def test_nothing_in_result_cf_is_discounted(uk_ip_claim):
    ...
    p = uk_ip_claim
    df = p.result_cf()
    assert df.loc[0, "claims_benefit"] == pytest.approx(1911.79, abs=PENNY)
    assert df.loc[0, "claims_benefit"] != pytest.approx(1907.09, abs=0.5)
    assert "disc_factor" not in df.columns


def test_the_disabled_life_annuity_factor(uk_ip_claim):
    """a_dis = pv_benefits / AP at t = 0, the notes' claims-in-payment liability per unit."""
    p = uk_ip_claim
    assert p.annuity_dis() == pytest.approx(
        p.pv_benefits() / p.amount_payable_pp(0), rel=1e-12)
    assert p.pv_benefits() == pytest.approx(
        sum(p.claims(t, "BENEFIT") * p.disc_factor(t)
            for t in range(p.proj_len())), rel=1e-12)
...
def test_the_discount_rate_is_a_reference_not_a_valuation_basis(income_protection):
    ...
    try:
        ...
        assert proj.disc_factor(0) == 1.0 and proj.disc_factor(120) == 1.0
        assert proj.pv_benefits() > base          # undiscounted is the upper bound
        assert proj.pv_benefits() == pytest.approx(
            sum(proj.claims(t, "BENEFIT") for t in range(proj.proj_len())),
            rel=1e-12)
    finally:
        model.close()
...
def test_result_cf_shape(uk_ip_claim):
    """One row per policy month, t = 0 .. proj_len() - 1, and the fixed column set."""
    df = uk_ip_claim.result_cf()
    assert df.index.name == "t"
    assert list(df.index) == list(range(360)) == list(range(uk_ip_claim.proj_len()))
    assert df.index[0] == 0 and df.index[-1] == uk_ip_claim.proj_len() - 1
    assert df.loc[0, "pols_if"] == uk_ip_claim.pols_if_init()
...
def test_expenses_carry_the_claim_management_load(income_protection):
    ...
    p = income_protection.Projection[1]
    for t in (0, 12, 99):
        expected = (60.0 / 12 * 1.03 ** (t // 12) * p.pols_if(t)
                    + 300.0 / 12 * 1.03 ** (t // 12) * p.pols_sick(t))
    ...
    assert claim.expenses(0) == pytest.approx((60.0 + 300.0) / 12, rel=1e-12)
    assert (300.0 / 12) / claim.expenses(0) == pytest.approx(300 / 360, rel=1e-12)
...
def test_round_trip_is_stable(tmp_path):
    ...
    try:
        ...
        for t, row in WORKED_EXAMPLE.items():
            assert claim.pols_sick(t) == pytest.approx(row[0], abs=STATE)
            assert claim.claims(t, "BENEFIT") == pytest.approx(row[4], abs=PENNY)
