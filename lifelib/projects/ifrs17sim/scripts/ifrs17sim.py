...
def build():
    ...
    baseproj = model.import_module(module="projection", name="BaseProj")

    ifrs = model.import_module(module="ifrs", name="IFRS")

    outerproj = model.new_space(
        bases=[ifrs, baseproj], name="OuterProj", formula=proj_params, refs=proj_refs
    )
    ...
    pvs = innerproj.import_module(module="present_value", name="PresentValue")

    def pvs_params(t_rate):
        refs = {
            "last_t": _space.parent.last_t,
