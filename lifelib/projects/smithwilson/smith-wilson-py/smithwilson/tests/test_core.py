...
class TestSmithWilson(unittest.TestCase):
    ...
    def test_ufr_discount_factor(self):
        ...
        # Expected Output
        expected = np.array(
            [0.992878614, 0.971817298, 0.866808430, 0.242906395, 0.028059385]
        )
        ...
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="UFR discount factors not matching"
        )
    ...
    def test_calculate_prices(self):
        ...
        # Expected Output
        expected = np.array(
            [0.995061577, 0.975609756, 1.182681027, 0.611071456, 0.904873593]
        )
        ...
        self.assertTupleEqual(actual.shape, expected.shape, "Shapes not matching")
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="Prices not matching"
        )
    ...
    def test_wilson_function_symmetric(self):
        ...
        self.assertTupleEqual(actual.shape, expected.shape, "Shapes not matching")
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="Wilson functions not matching"
        )
    ...
    def test_wilson_function_asymmetric_t1_lt_t2(self):
        ...
        t_target = np.array(
            [0.25, 0.5, 1.0, 2.0, 2.5, 3.5, 5.0, 10.0, 20.0, 49.5, 125.0]
        ).reshape((-1, 1))
        ufr = 0.029
        alpha = 0.2
    ...
    def test_fit_parameters(self):
        ...
        # Expected Output
        expected = np.array(
            [-42.78076209, 23.4627511, -3.96498616, 8.92604195, -75.22418515]
        ).reshape((-1, 1))
        ...
        self.assertTupleEqual(actual.shape, expected.shape, "Shapes not matching")
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="Parameter not matching"
        )
    ...
    def test_fit_smithwilson_rates_actual(self):
        ...
        t = np.array([float(y + 1) for y in range(len(r))]).reshape(
            (-1, 1)
        )  # 1.0, 2.0, ..., 25.0
        ufr = 0.029
        alpha = 0.128562
        ...
        # Actual Output
        actual = sw.fit_smithwilson_rates(
            rates_obs=r, t_obs=t, t_target=t_target, ufr=ufr, alpha=alpha
        )
        ...
        self.assertTupleEqual(actual.shape, expected.shape, "Shapes not matching")
        np.testing.assert_almost_equal(
            actual, expected, decimal=4, err_msg="Fitted rates not matching"
        )
    ...
    def test_fit_smithwilson_rates_random(self):
        ...
        self.assertTupleEqual(actual.shape, expected.shape, "Shapes not matching")
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="Fitted rates not matching"
        )
