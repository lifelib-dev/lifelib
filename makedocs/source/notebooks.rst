
.. _notebooks_index:

Jupyter Notebooks
=================

.. _notebooks_ifrs17sim:

:mod:`ifrs17sim` Notebooks
--------------------------

.. include:: /binderlinks.rst


.. toctree::
   :hidden:

   projects/ifrs17sim_csm_waterfall.ipynb
   projects/ifrs17sim_ifrs_waterfall.ipynb


* `CSM Waterfall Chart <projects/ifrs17sim_csm_waterfall.ipynb>`_  (*Run online now!* |binder ifrs17sim_csm_waterfall| )
* `IFRS17 Waterfall Charts <projects/ifrs17sim_ifrs_waterfall.ipynb>`_  (*Run online now!* |binder ifrs17sim_csm_waterfall| )
